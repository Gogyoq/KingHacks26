...
# ===== STORY PLAN MODELS (for objectives/progress tracking) =====

class StoryNode(BaseModel):
    """A single node in the story plan"""
    id: str
    type: str  # "story" or "problem"
    content_goal: str  # What this node should teach/ask
    problem_data: dict | None = None  # For problem nodes: expected_answer, hint
    objective_index: int = 0  # Which objective this belongs to

class StoryPlan(BaseModel):
    """The full story plan generated by the architect"""
    nodes: list[StoryNode]
    total_problems: int = 0
    current_node_index: int = 0
    objectives: list[str] = []  # e.g., ["Introduction", "Practice", "Challenge"]

# Default objectives for lessons
DEFAULT_OBJECTIVES = [
    {"name": "Introduction", "problems": 2},
    {"name": "Practice", "problems": 3},
    {"name": "Challenge", "problems": 2},
]
...
def save_story_plan(conversation_id: int, plan: StoryPlan):
    """Save the story plan to the conversation."""
    try:
        conn = sqlite3.connect('chat_history.db')
        c = conn.cursor()
        c.execute("""
            UPDATE student_conversations
            SET story_plan = ?, total_problems = ?, current_objective_index = 0, problems_completed = 0
            WHERE id = ?
        """, (plan.model_dump_json(), plan.total_problems, conversation_id))
        conn.commit()
        conn.close()
    except Exception as e:
        print(f"Error saving story plan: {e}")

def get_story_plan(conversation_id: int) -> StoryPlan | None:
    """Get the story plan from a conversation."""
    try:
        conn = sqlite3.connect('chat_history.db')
        conn.row_factory = sqlite3.Row
        c = conn.cursor()
        c.execute("SELECT story_plan FROM student_conversations WHERE id = ?", (conversation_id,))
        row = c.fetchone()
        conn.close()
        if row and row['story_plan']:
            return StoryPlan.model_validate_json(row['story_plan'])
        return None
    except Exception as e:
        print(f"Error getting story plan: {e}")
        return None

def update_progress(conversation_id: int, problems_completed: int, current_objective_index: int):
    ...
    try:
        ...
        c.execute("""
            UPDATE student_conversations
            SET problems_completed = ?, current_objective_index = ?
            WHERE id = ?
        """, (problems_completed, current_objective_index, conversation_id))
        conn.commit()
        conn.close()
    except Exception as e:
...
def get_progress(conversation_id: int) -> dict:
    """Get progress info for a conversation."""
    try:
        conn = sqlite3.connect('chat_history.db')
        conn.row_factory = sqlite3.Row
        c = conn.cursor()
        c.execute("""
            SELECT problems_completed, total_problems, current_objective_index, story_plan
            FROM student_conversations WHERE id = ?
        """, (conversation_id,))
        row = c.fetchone()
        conn.close()
        if row:
            # Get objective name from story plan
            objective_name = "Learning"
            if row['story_plan']:
                try:
                    plan = StoryPlan.model_validate_json(row['story_plan'])
                    if plan.objectives and row['current_objective_index'] < len(plan.objectives):
                        objective_name = plan.objectives[row['current_objective_index']]
                except:
                    pass

            total = row['total_problems'] or 7  # Default to 7 if not set
            completed = row['problems_completed'] or 0
            return {
                'problems_completed': completed,
                'total_problems': total,
                'current_objective_index': row['current_objective_index'] or 0,
        ...
        try:
            ...
            if not current_thread_id:
                ...
                if user_id:
                    ...
                    # Initialize progress with default objectives
                    total_problems = sum(obj["problems"] for obj in DEFAULT_OBJECTIVES)
                    default_plan = StoryPlan(
                        nodes=[],
                        total_problems=total_problems,
                        current_node_index=0,
                        objectives=[obj["name"] for obj in DEFAULT_OBJECTIVES]
                    )
                    save_story_plan(conversation_id, default_plan)
...
{{"story": "your story here", "question": "your question here", "expected_answer": "the answer", "difficulty": "easy", "hint": "a helpful hint"}}"""
                ...
                # Get progress for first message
                first_progress = get_progress(conversation_id) if conversation_id else None

                # Stream the display text to the frontend with progress
                first_sse_data = {
                    'type': 'content',
                ...
                if first_progress:
                    first_sse_data['progress'] = first_progress
                yield f"data: {json.dumps(first_sse_data)}\n\n"
...
{{"story": "your story here", "question": "your question here", "expected_answer": "the answer", "difficulty": "easy/medium/hard", "hint": "a helpful hint"}}"""
                    ...
                    # Update progress - increment problems completed
                    if conversation_id:
                        current_progress = get_progress(conversation_id)
                        new_completed = current_progress['problems_completed'] + 1
                        # Calculate new objective index based on completed problems
                        cumulative = 0
                        new_objective_index = 0
                        for i, obj in enumerate(DEFAULT_OBJECTIVES):
                            cumulative += obj["problems"]
                            if new_completed <= cumulative:
                                new_objective_index = i
                                break
                            new_objective_index = i + 1
                        update_progress(conversation_id, new_completed, new_objective_index)
                ...
                # Get current progress for SSE
                progress_data = get_progress(conversation_id) if conversation_id else None

                # Stream the response with difficulty and progress info
                sse_data = {
                    'type': 'content',
                    'content': display_text,
                    'thread_id': str(current_thread_id),
                ...
                if progress_data:
                    sse_data['progress'] = progress_data
            ...
            # Get final progress for done signal
            final_progress = get_progress(conversation_id) if conversation_id else None

            # Send done signal with progress
            done_data = {
                'type': 'done',
                'thread_id': str(current_thread_id),
                'conversation_id': conversation_id,
                'was_wrong': is_wrong_answer
            }
            if final_progress:
                done_data['progress'] = final_progress
...
async def get_conversation_progress(conversation_id: int, authorization: Optional[str] = Header(None)):
    """Get progress for a specific conversation."""
    user_id = get_user_id_from_token(authorization) if authorization else None
    if not user_id:
    ...
    progress = get_progress(conversation_id)
    return progress
